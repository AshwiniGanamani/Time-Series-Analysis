...
#oilseries<- read.csv("//Users//ashwinichowdhary//Documents//DataSets//oilsp.csv", 
...
ts.all<-ts(oilseries)
ts.all
ts.1<-ts(oilseries$Adj.Close)    # to select one series only
ts.1
```
...
```{r}
ts.quart<-ts(oilseries$Adj.Close, frequency = 4) 
ts.quart
ts.month<-ts(oilseries$Adj.Close, frequency = 12) 
ts.month
```
...
```{r}
ts.4<-ts(oilseries$Adj.Close, frequency = 12)
plot.ts(ts.4)
...
head(rain)
str(rain)
series <- ts(oilseries,start=c(2012), frequency=2)
plot.ts(series)


series.d <- decompose(series)
plot(rainseries.d)
